...
- Chlorophyll is a stack of three satellite layers shown together so swath gaps and cloud holes in one are filled by the others: OCI_PACE_Chlorophyll_a, VIIRS_NOAA21_Chlorophyll_a, VIIRS_NOAA20_Chlorophyll_a. (Originally a single MODIS_Aqua_Chlorophyll_A layer, but NASA retired MODIS Aqua and its feed stopped publishing, so it was replaced July 2026.)
- Chlorophyll tiles render through FilteredChlTiles (canvas L.GridLayer) which runs filterChlImage: (1) purple palette bins (near zero chl, r>90 b>90 g<70) are always dropped as bad retrievals, and (2) any contiguous opaque data patch smaller than 200 px (~60 sq km at z7) is dropped, because a real satellite pass is contiguous and tiny data islands are noise. Lower layers or the previous day fill the holes. The Fronts pipeline runs the same filter on its chl mosaic so noise cannot create fake edges. The chl stack also renders in its own map pane at full per-layer opacity with the pane faded to 0.68, so overlapping translucent layers cannot blend into false colors (that was causing phantom purple). Temp layer: GHRSST_L4_MUR_Sea_Surface_Temperature. Tile matrix GoogleMapsCompatible_Level7 (coarse, a few km per pixel, good for spotting breaks, not fine detail).
...
- Fronts button: client side edge detection on the GIBS imagery itself (canvas, crossOrigin anonymous, GIBS sends CORS). Always computed at z7, the imagery's full native resolution (about 1 km pixels; overlays also display at maxNativeZoom 7). SST mosaic threshold 35 no blur; chlorophyll stack mosaic gets a 5x5 alpha-aware box blur then threshold 60; gradients are measured across a 2 px step (same physical distance as 1 px at z6, so thresholds carried over from the z6 version); both edge maps are despeckled (keep pixel only with 2+ of 8 neighbors). Red = temp break, green = color break, purple = both within 2 px. Rendered as an L.imageOverlay for the current view; user rescans manually after panning. Gradients are never computed across transparent (no data) pixels so cloud boundaries do not read as fronts. If the view is too zoomed out the z7 mosaic exceeds the 4096 px cap and the app asks the user to zoom in.
